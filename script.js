...
function seed() {
  ...
  const idea9 = mk({
    ...
    status: 'concluida', prioridade: 'normal', decisao: 'aprovada',
...
function kpiCard(icon_, label, value, cls) {
  return `<div class="kpi-card"><div class="kpi-top"><span class="kpi-icon">${icon(icon_)}</span></div><div class="kpi-num" style="${cls ? `color:var(--${cls})` : ''}">${value}</div><div class="kpi-label">${label}</div></div>`;
}
...
function renderColabHome() {
  ...
  <div class="kpi-grid">
    ${kpiCard('send', 'Ideias Enviadas', mine.length)}
    ${kpiCard('badge-check', 'Aprovadas', aprovadas)}
    ${kpiCard('clock', 'Em Análise', emAnalise)}
    ${kpiCard('circle-dollar-sign', 'Economia Estimada', fmtMoney(economiaEstimada))}
  </div>
...
function renderComiteDashboard() {
  ...
  return `
  <div class="kpi-grid">
    ${kpiCard('list-checks', 'Ideias aguardando parecer', aguardando)}
    ${kpiCard('calendar-check', 'Reunião de hoje', DB.reunioes.some(r => r.data === new Date().toISOString().slice(0, 10)) ? 'Sim' : 'Nenhuma')}
    ${kpiCard('history', 'Ideias já analisadas', analisadas)}
    ${kpiCard('rocket', 'Implantações acompanhadas', implant)}
  </div>
  <div class="grid-2">
    <div class="panel">
...
function renderQualDashboard() {
  ...
  return `
  <div class="kpi-grid">
    ${kpiCard('inbox', 'Aguardando análise', aguardandoAnalise)}
    ${kpiCard('pencil', 'Cartas aguardando elaboração', cartasElaboracao)}
    ${kpiCard('file-check-2', 'Aguardando Analista', cartasAnalista)}
    ${kpiCard('briefcase', 'Aguardando Gerente', cartasGerente)}
  </div>
  <div class="kpi-grid">
    ${kpiCard('landmark', 'Aguardando Diretoria', cartasDiretoria)}
    ${kpiCard('rocket', 'Implantações em andamento', implantAndamento)}
    ${kpiCard('activity', 'Acompanhamentos ativos', acompAtivos)}
    ${kpiCard('badge-check', 'Concluídas', ideias.filter(i => i.status === 'concluida').length)}
  </div>
...
/* ============================================================
   ...
   Este é um personagem ORIGINAL, desenhado para este protótipo —
   não reproduz o mascote de nenhum outro aplicativo. A ideia é a
   mesma de assistentes flutuantes conhecidos (um "amigo" que dá
   dicas rápidas): um sinal de trânsito circular sorridente, nas
   cores do programa. Quando a identidade visual definitiva do
   "Univaldo" estiver pronta, basta substituir o SVG em
   MASCOT_SVG e os textos em MASCOT_NAME / MASCOT_ROLE abaixo —
   toda a lógica (abrir, fechar, trocar de dica) continua igual.
   ============================================================ */

const MASCOT_NAME = 'Univaldo';
const MASCOT_ROLE = 'seu guia no Sinal Verde';

// Sinal de trânsito circular animado, com "rosto" e um braço que
// acena — 100% vetor (SVG), sem depender de nenhuma imagem externa.
// Os olhos piscam e o braço aceno via CSS (ver seção MASCOTE no style.css).
const MASCOT_SVG = `
<svg viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg">
  <defs>
    <radialGradient id="mascotHeadGrad" cx="35%" cy="30%" r="75%">
      <stop offset="0%" stop-color="#5FCB9E"/>
      <stop offset="100%" stop-color="var(--primary)"/>
    </radialGradient>
  </defs>
  <ellipse cx="32" cy="59" rx="13" ry="2.6" fill="rgba(0,0,0,.14)"/>
  <circle cx="32" cy="32" r="30" fill="#FFFFFF"/>
  <circle cx="32" cy="32" r="30" fill="none" stroke="var(--univale-green-dark)" stroke-width="2.5"/>
  <rect class="mascot-arm-l" x="9" y="39" width="8" height="4.5" rx="2.2" fill="url(#mascotHeadGrad)"/>
  <rect class="mascot-arm-r" x="47" y="39" width="8" height="4.5" rx="2.2" fill="url(#mascotHeadGrad)"/>
  <circle cx="32" cy="24" r="13" fill="url(#mascotHeadGrad)"/>
  <circle class="mascot-eye" cx="27.5" cy="21.5" r="2.4" fill="#FFFFFF"/>
  <circle class="mascot-eye" cx="36.5" cy="21.5" r="2.4" fill="#FFFFFF"/>
  <path d="M25 27c2.2 2.4 11.8 2.4 14 0" stroke="#FFFFFF" stroke-width="2" stroke-linecap="round" fill="none"/>
  <rect x="22" y="42" width="20" height="7" rx="3.5" fill="var(--primary-light)"/>
  <rect x="24" y="52" width="16" height="6" rx="3" fill="var(--primary-light)"/>
</svg>`;
...
function mascotInit() {
  ...
  fab.innerHTML = `<span class="mascot-fab-inner">${MASCOT_SVG}</span>`;
